...
for j in range(1,2048):
	...
	if rank == j:
		#j = 0
		data = np.loadtxt('/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/radecz_files/04/S/Patchy-Mocks-DR12%s-COMPSAM_V6C_%04d.dat' % (hemi, j))
		if region == 1:
		...
		np.savetxt('/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/reg%i/04/S/Patchy-Mocks-DR12%s_reg%i-COMPSAM_V6C_%04d_cart.data' % (region, hemi, region, j),out)
		print(j)

		data = np.loadtxt('/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/radecz_files/Patchy-Mocks-Randoms-DR12%s-COMPSAM_V6C_x50.dat' % hemi)
		if region == 1:
		...
		for i in range(32):
			if (i == 0) or (j == 1):
				...
				np.savetxt('/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/reg%i/04/S/Patchy-Mocks-DR12%s_reg%i-COMPSAM_V6C_x50_%04d_cart.ran.%02d' % (region, hemi, region, j, i),out)
