...
data = fits.open('/gpfs/akrolewski/parity_odd_4pcf/data/galaxy_DR12v5_CMASS_South.fits.gz')[1].data
...
np.savetxt('/gpfs/akrolewski/parity_odd_4pcf/data/galaxy_DR12v5_CMASS_South_cart.data',out)
#print(j)

data = fits.open('/gpfs/akrolewski/parity_odd_4pcf/data/random0_DR12v5_CMASS_South.fits.gz')[1].data
data = data[(data['Z'] >= zmin) & (data['Z'] <= zmax) ]
# if region == 1:
# 	data = data[(data['Z'] >= zmin) & (data['Z'] <= zmax) 
# 		& (data['RA']< (155 - 1./12. * data['DEC']))]
...
for i in range(32):
	...
	np.savetxt('/gpfs/akrolewski/parity_odd_4pcf/data/random0_DR12v5_CMASS_South_cart.ran.%02d' % (i),out)
