...
#SBATCH -p preq
#SBATCH --dependency=afterany:106465
...
root="Patchy-Mocks-DR12NGC_reg3-COMPSAM_V6C_${ID}_cart" # root for data filenames
ranroot=Patchy-Mocks-DR12NGC_reg3-COMPSAM_V6C_x50_0001_cart # root for random filenames
in=/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/reg3/04/ # input directory (see above for required contents)
out="/gpfs/akrolewski/parity_odd_4pcf/out/10_bins/patchy_mocks_reg3/N/${SLURM_ARRAY_TASK_ID}/" # output file directory
tmp="/gpfs/akrolewski/scratch/patchy_mocks_reg3/${ID}" # temporary directory for intermediate file storage for this run (ideally somewhere with fast I/O)
...
if (($lineflag == 5)); then
    ...
    cp ${in}/Patchy-Mocks-DR12NGC_reg3-COMPSAM_V6C_x50_${ID}_cart.ran.00 $tmp/$root.ran.00
    echo $lineflag >> $out/progress.txt
    export lineflag=$(($lineflag + 1))
fi
...
do
	...
	cat "$in/$root.data" "/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/reg3/00/$ranroot.ran.${n}" > "$tmp/$root.ran.${n}"
