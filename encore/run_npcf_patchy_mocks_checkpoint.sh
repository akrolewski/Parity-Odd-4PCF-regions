...
out="/gpfs/akrolewski/parity_odd_4pcf/out/10_bins/patchy_mocks/N/${SLURM_ARRAY_TASK_ID}/" # output file directory
tmp="/gpfs/akrolewski/scratch/${ID}" # temporary directory for intermediate file storage for this run (ideally somewhere with fast I/O)
...
do
    ...
    if (($lineflag == $((10#$n*3 + 8)))); then
        cat "$in/$root.data" "/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/cart_files_default/00/$ranroot.ran.${n}" > "$tmp/$root.ran.${n}"
        echo $lineflag >> $out/progress.txt
        export lineflag=$(($lineflag + 1))
    fi
