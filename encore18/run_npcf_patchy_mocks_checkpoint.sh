...
##########################################################
#SBATCH --array=2,28,44,99
#SBATCH -n 64 # cpus
#SBATCH -t 0-06:00:00 # time
...
#SBATCH -p preq
#SBATCH --dependency=afterany:113388
...
ngrid=51 # grid-size for accelerating pair count
...
root="Patchy-Mocks-DR12NGC-COMPSAM_V6C_${ID}_cart" # root for data filenames
ranroot=Patchy-Mocks-DR12NGC-COMPSAM_V6C_x50_0001_cart # root for random filenames
in=/gpfs/akrolewski/parity_odd_4pcf/data/patchy_mocks/cart_files_default/00/ # input directory (see above for required contents)
out="/gpfs/akrolewski/parity_odd_4pcf/out/18_bins/patchy_mocks/N/${SLURM_ARRAY_TASK_ID}/" # output file directory
tmp="/gpfs/akrolewski/scratch/18_bins/patchy_mocks/N/${ID}" # temporary directory for intermediate file storage for this run (ideally somewhere with fast I/O)
...
if (($lineflag == 5)); then
    ...
    cp ${in}/Patchy-Mocks-DR12NGC-COMPSAM_V6C_x50_${ID}_cart.ran.00 $tmp/$root.ran.00
    echo $lineflag >> $out/progress.txt
    export lineflag=$(($lineflag + 1))
fi
